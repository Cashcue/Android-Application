...
class RegisterViewModel(private val userRepository: UserRepository): ViewModel() {
    ...
    fun register(nama: String, email: String, password: String): LiveData<Result<String>> = liveData {
        emit(Result.Loading)
        try {
            val response = ApiConfig.getApiService().register(nama, email, password)
